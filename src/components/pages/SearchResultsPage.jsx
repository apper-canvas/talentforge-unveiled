        ...
        <motion.div
            ...
            <div className="glass rounded-2xl p-8">
                ...
                <div
                    ...
                    {(searchQuery || selectedCategory !== "all") && <button
                        onClick={clearFilters}
                        className="flex items-center gap-2 px-4 py-2 text-sm text-slate-400 hover:text-white transition-colors">
                        <ApperIcon name="X" className="w-4 h-4" />Clear Filters
                                        </button>}
                </div>
            </div>
        </motion.div>
        {/* Results */}
        <FreelancerGrid searchQuery={searchQuery} selectedCategory={selectedCategory} />
    </div></div>
  )
}
